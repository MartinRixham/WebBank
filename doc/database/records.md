...
records consistent with each other — that is a [batch](/database/batches), and a
batch is unconditional. A conditional multi-key write is the one thing a client
may reasonably want that this API does not have; if it turns out to be needed,
it is a transaction resource, not another header.

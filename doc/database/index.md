...
- Writing several records **atomically**, across tables.
- Reading a **consistent view** across several requests.
...
| Batch | `/batch`, `/table/{table}/batch` | An atomic group of writes |
| Snapshot | `/snapshot/{id}` | A view of the instance at a point in time |
...
list — `records`, `operations`, `tables` — because it names a part of a
document, not a resource, and calling an array `record` would say the wrong
thing about what is inside it.

Read the pages in that order: [tables](/database/tables),
[records](/database/records), [scans](/database/scans),
[batches](/database/batches), and the [reference](/database/reference) for
status codes, error codes and limits.
...
Writes to different tables are still atomic together, because column families
share one write-ahead log. That is what makes [batches](/database/batches) worth
having.
...
The `database` module in this repository puts a JSON API in front of Redis
today, with two endpoints of its own — `POST /account` and `GET /accounts`.
Those are the account store's endpoints, not a database's: they know what an
account is.

On this API the same store is a table:

| Today | On this API |
| --- | --- |
| `POST /account` with an account body | `PUT /table/account/key/{id}` with the same body |
| `GET /accounts` | `GET /table/account/key` |
| Redis hash `account:{id}` | Key `{id}` in table `account` |

The account service keeps its own endpoints and becomes a client of this one,
which is what moves the knowledge of what an account is out of the store.
Whether the two run as one process or two is a deployment question, not an API
one.
